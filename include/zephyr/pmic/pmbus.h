...
static inline int pmbus_set_page(const struct device *dev, uint8_t addr, uint8_t page)
{
    if (!device_is_ready(dev)) {
        ...
        printf("device is ready and page number is =%d\n",page);
                                       ...
                                       uint8_t reg, uint8_t *value)
{
    ...
    if (page != PMBUS_NO_PAGE) {
        ...
        if (ret) {
		printk("read is not called and terminated\n");
            return ret;
        }
    }
    printk("called\n");
...
{
    int ret;
    printk("entering into the write\n");
    if (!device_is_ready(dev)) {
        return -ENODEV;
    }
    printk("D ready\n");
    ...
    if (page != PMBUS_NO_PAGE) {
	    printk("page set is called\n");
        ret = pmbus_set_page(dev, addr, page);
        if (ret) {
		printk("page set is failed\n");
            return ret;
        }
    }
