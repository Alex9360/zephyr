...
static int soc_start_cm7(void)
{
	...
	{
		reset_vec = *(volatile uint32_t *)(DT_REG_ADDR(DT_NODELABEL(flash_m7_0)) + 4);
		if (reset_vec != 0xFFFFFFFFU) {
	...
	{
		reset_vec = *(volatile uint32_t *)(DT_REG_ADDR(DT_NODELABEL(flash_m7_1)) + 4);
		if (reset_vec != 0xFFFFFFFFU) {
			Cy_SysEnableCM7(CORE_CM7_1, DT_REG_ADDR(DT_NODELABEL(flash_m7_1)));
		} else {
			printf("M7_1 release skipped - no firmware at flash_m7_1");
		}
	}
#endif
	return 0;
