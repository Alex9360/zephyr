...
static int ifx_cat1_flash_read(const struct device *dev, off_t offset,
			       void *data, size_t len)
{
	...
	if ((uint32_t)offset > cfg->region_size) {
		return -EINVAL;
	}

	if ((cfg->region_size - (uint32_t)offset) < len) {
		return -EINVAL;
	}

	addr = cfg->base_addr + (uint32_t)offset;

	ifx_cache_invalidate(cfg->base_addr + (uint32_t)offset,
			     (size_t)(addr - (cfg->base_addr + (uint32_t)offset)));
...
static int ifx_cat1_flash_write(const struct device *dev, off_t offset,
				const void *data, size_t len)
{
	...
	if ((offset % cfg->write_block_size) != 0 ||
	    (len % cfg->write_block_size) != 0) {
		return -EINVAL;
	}
...
static int ifx_cat1_flash_erase(const struct device *dev, off_t offset,
				size_t size)
{
	const struct ifx_cat1_flash_config *cfg = dev->config;
	struct ifx_cat1_flash_data *dev_data = dev->data;
	...
	if ((cfg->region_size - (uint32_t)offset) < size) {
		return -EINVAL;
	}
