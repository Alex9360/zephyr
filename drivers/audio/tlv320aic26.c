...
static int aic26_poll_powerup(const struct device *dev, uint16_t flag_bit)
{
	uint16_t reg;
	int ret;
	int i;

	for (i = 0; i < AIC26_POLL_TIMEOUT_MS / AIC26_POLL_INTERVAL_MS; i++) {
		k_msleep(AIC26_POLL_INTERVAL_MS);

		ret = aic26_reg_read(dev, AIC26_PAGE2,
	...
	LOG_WRN("Power-up timeout (flag=0x%04x reg=0x%04x)", flag_bit, reg);
	return 0;
}
...
static int aic26_adc_power_up(const struct device *dev)
{
	...
	ret = aic26_reg_update(dev, AIC26_PAGE2, AIC26_P2_AUDIO_CTL3,
			       AIC26_AGCNL,
			       FIELD_PREP(AIC26_AGCNL, AIC26_AGCNL_M60));
	if (ret < 0) {
		return ret;
	}
	...
	return aic26_poll_powerup(dev, AIC26_ADPWDF);
}
			   ...
			   struct audio_codec_cfg *cfg)
{
	...
	if (!!(cfg->dai_cfg.i2s.options & I2S_OPT_BIT_CLK_SLAVE) !=
	    !!(cfg->dai_cfg.i2s.options & I2S_OPT_FRAME_CLK_SLAVE)) {
		return -EINVAL;
	}

	codec_master = !(cfg->dai_cfg.i2s.options & I2S_OPT_BIT_CLK_SLAVE);
...
static void aic26_start_output(const struct device *dev)
{
	...
	aic26_poll_powerup(dev, AIC26_DAPWDF);

	aic26_reg_write(dev, AIC26_PAGE2, AIC26_P2_DAC_GAIN, 0x0000);
}
...
static void aic26_stop_output(const struct device *dev)
{
	...
	mute_val = AIC26_DALMU |
		   FIELD_PREP(AIC26_DALVL, 0x7F) |
		   AIC26_DARMU |
		   FIELD_PREP(AIC26_DARVL, 0x7F);

	aic26_reg_write(dev, AIC26_PAGE2, AIC26_P2_DAC_GAIN, mute_val);
