...
struct cat1_intc_dispatcher {
	uint32_t nvic_line;
};

#define _CAT1_DECLARE_DISPATCHER(node_id)		\
	{						\
		.nvic_line = DT_REG_ADDR(node_id),	\
	}

#define CAT1_DECLARE_DISPATCHERS(parent_id) \
	DT_FOREACH_CHILD_STATUS_OKAY_SEP(parent_id, _CAT1_DECLARE_DISPATCHER, (,))

static struct cat1_intc_dispatcher dispatchers[] = {
	CAT1_DECLARE_DISPATCHERS(DT_NODELABEL(cpuss_intc))
};

static void cat1_intc_mux_isr(const void *data)
{
	const struct cat1_intc_dispatcher *disp = data;
	cy_en_intr_t sys_int;
	unsigned int zephyr_irq;
	uint32_t table_idx;

	sys_int = Cy_SysInt_GetInterruptActive((IRQn_Type)disp->nvic_line);
	if (sys_int == CY_CPUSS_NOT_CONNECTED_IRQN) {
		return;
	}

	zephyr_irq = irq_to_level_2((unsigned int)sys_int) | disp->nvic_line;
	table_idx = z_get_sw_isr_table_idx(zephyr_irq);

	_sw_isr_table[table_idx].isr(_sw_isr_table[table_idx].arg);
}

/* Register dispatcher ISR on each parent NVIC line at build time */
#define _CAT1_REGISTER_DISPATCHER(node_id)			\
	IRQ_CONNECT(DT_IRQN(node_id),				\
		    DT_IRQ(node_id, priority),			\
		    cat1_intc_mux_isr,				\
		    &dispatchers[DT_NODE_CHILD_IDX(node_id)],	\
		    0)
...
#define CAT1_MUX_IRQ_PARENT_ENTRY(node_id)				\
	IRQ_PARENT_ENTRY_DEFINE(					\
		CONCAT(cat1_aggregator_, DT_NODE_CHILD_IDX(node_id)),	\
		NULL,							\
		DT_IRQN(node_id),					\
		INTC_CHILD_ISR_TBL_OFFSET(node_id),			\
