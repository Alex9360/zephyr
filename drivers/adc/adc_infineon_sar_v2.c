...
static void ifx_cat1_sar_isr(const struct device *dev)
{
	...
	config->base->CH[last_ch].TR_CTL &=
		~PASS_SAR_CH_TR_CTL_GROUP_END_Msk;
...
{
	struct ifx_cat1_sar_data *data =
		CONTAINER_OF(ctx, struct ifx_cat1_sar_data, ctx);
...
static void adc_context_start_sampling(struct adc_context *ctx)
{
	...
	Cy_SAR2_Channel_ClearInterrupt(config->base, last_ch,
				       CY_SAR2_INT_GRP_DONE);
	Cy_SAR2_Channel_SetInterruptMask(config->base, last_ch,
					 CY_SAR2_INT_GRP_DONE);
				      ...
				      const struct adc_channel_cfg *channel_cfg)
{
	...
	if (channel_cfg->acquisition_time != ADC_ACQ_TIME_DEFAULT) {
		...
		case ADC_ACQ_TIME_TICKS:
			sample_time = ADC_ACQ_TIME_VALUE(
					channel_cfg->acquisition_time);
			break;
		case ADC_ACQ_TIME_MICROSECONDS:
			sample_time = ADC_ACQ_TIME_VALUE(
					channel_cfg->acquisition_time) *
				      (config->frequency / SAR_CLK_PER_US);
			break;
		case ADC_ACQ_TIME_NANOSECONDS:
			sample_time = (ADC_ACQ_TIME_VALUE(
					channel_cfg->acquisition_time) / 1000) *
				      (config->frequency / SAR_CLK_PER_US);
			break;
		default:
...
static int ifx_cat1_sar_hw_init(const struct device *dev)
{
	...
	sar_cfg.powerupTime = PWR_DELAY_MS * (config->frequency / SAR_CLK_PER_US);
...
static int ifx_cat1_sar_init(const struct device *dev)
{
	...
	if (config->frequency < SAR_MIN_FREQ_HZ ||
	    config->frequency > SAR_MAX_FREQ_HZ) {
...
#define ADC_PERI_CLOCK_INIT(n)						\
	.clk_info = {							\
		.rootclk_id = DT_INST_CLOCKS_CELL_BY_IDX(n, 0,		\
							  rootclk_id),	\
		.divider_type = DT_INST_CLOCKS_CELL_BY_IDX(n, 0,	\
							   divider_type), \
		.divider_inst = DT_INST_CLOCKS_CELL_BY_IDX(n, 0,	\
							   divider_inst), \
	},
...
#define IFX_SAR_ADC_INIT(n)						\
	...
	static const struct ifx_cat1_sar_config				\
		...
		.frequency = DT_INST_PROP_OR(n, clock_frequency,	\
					     SAR_MAX_FREQ_HZ),		\
