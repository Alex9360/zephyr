...
static int comparator_mspm0_init(const struct device *dev)
{
	...
	data->window_mode_active = config->window_mode_enable;
	...
	if (data->window_mode_active && config->window_companion_regs) {
		DL_COMP_enableWindowComparator(config->regs);
		DL_COMP_setPositiveChannelInput(config->regs, config->window_signal_input);
		DL_COMP_setNegativeChannelInput(config->regs, config->window_upper_thresh);
...
static const struct comparator_mspm0_config comparator_mspm0_config_##n = { \
	...
	.window_companion_regs = COND_CODE_1(\
			...
			((COMP_Regs *)DT_REG_ADDR(DT_PHANDLE(DT_DRV_INST(n), window_companion))),\
			(NULL)), \
