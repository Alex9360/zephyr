...
#define I2S_TONE_HELP                                                                              \
	SHELL_HELP("Stream a generated test tone over I2S TX",                                     \
		   "<i2s-device> [frequency_hz] [sample_rate] [bits:16|24|32]")

#define I2S_STOP_HELP SHELL_HELP("Stop streaming the test tone", "")

#define I2S_INFO_HELP SHELL_HELP("Show I2S tone stream status", "")
...
static void i2s_shell_feeder(void *p1, void *p2, void *p3)
{
	...
	i2s_trigger(tone_stream.dev, I2S_DIR_TX,
...
static int cmd_tone(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (tone_stream.running) {
		shell_error(sh, "Already streaming on %s; 'i2s stop' first", tone_stream.dev->name);
		return -EBUSY;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_i2s,
	SHELL_CMD_ARG(tone, &dsub_device_name, I2S_TONE_HELP, cmd_tone, 2, 3),
	SHELL_CMD_ARG(stop, NULL, I2S_STOP_HELP, cmd_stop, 1, 0),
	SHELL_CMD_ARG(info, NULL, I2S_INFO_HELP, cmd_info, 1, 0),
