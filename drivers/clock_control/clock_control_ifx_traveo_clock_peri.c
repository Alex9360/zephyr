						 ...
						 clock_control_subsys_rate_t rate)
{
	...
	ret = Cy_SysClk_PeriPclkDisableDivider(clock_id, clk->divider_type, clk->divider_inst);
	if (ret != 0) {
		goto out;
	}
	...
	case IFX_PERI_DIV_8:
	case IFX_PERI_DIV_16:
		ret = Cy_SysClk_PeriPclkSetDivider(clock_id, clk->divider_type, clk->divider_inst,
						   int_div - 1U);
		break;
	case IFX_PERI_DIV_16_5:
	case IFX_PERI_DIV_24_5:
		uint8_t frac_div = (((float)data->src_clk_freq / (float)*clk_rate) - int_div) *
				   IFX_FRACTION_DIVIDER_VALUE;
		...
		ret = Cy_SysClk_PeriPclkSetFracDivider(clock_id, clk->divider_type,
						       clk->divider_inst, int_div - 1U, frac_div);
		break;
	default:
		ret = -EINVAL;
		goto out;
	...
	ret = Cy_SysClk_PeriPclkEnableDivider(clock_id, clk->divider_type, clk->divider_inst);
	if (ret != 0) {
		goto out;
	}

	ret = Cy_SysClk_PeriPclkAssignDivider(clock_id, clk->divider_type, clk->divider_inst);
	if (ret != 0) {
		goto out;
	}
