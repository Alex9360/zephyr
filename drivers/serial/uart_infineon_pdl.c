...
static int ifx_cat1_uart_init(const struct device *dev)
{
	...
	if (data->async.dma_rx.dma_dev != NULL) {
		...
		Cy_TrigMux_Select(TRIG_OUT_1TO1_1_SCB_RX_TO_PDMA10 +
				  (data->hw_resource.block_num * 2), false, TRIGGER_TYPE_EDGE);
#endif
	}
...
#elif defined(CONFIG_INFINEON_TRAVEO_FAMILY)
		Cy_TrigMux_Select(TRIG_OUT_1TO1_1_SCB_TX_TO_PDMA10 +
				  (data->hw_resource.block_num * 2), false, TRIGGER_TYPE_EDGE);
#endif
	}
